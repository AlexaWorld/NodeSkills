...
const intentHandler = function (alexaRequest) {
	...
	var key = this.textKey || requestName.replace('.', '_');
...
function getHandler(skill, textKey, shouldAsk = false) {
	var context = {
		skill: skill,
		textKey: textKey,
		shouldAsk: shouldAsk
...
module.exports.AddToSkill = function (skill) {
	skill.AddIntentHandler('AMAZON.HelpIntent', getHandler(skill, null, true));
	skill.AddIntentHandler('AMAZON.CancelIntent', getHandler(skill, "EndMessage"));
	skill.AddIntentHandler('AMAZON.StopIntent', getHandler(skill), "EndMessage");
